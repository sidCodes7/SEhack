...
- [ ] Create Expo app inside `apps/mobile/` (see prerequisite commands above)
- [ ] Configure `app.json` (name: "Aether", slug: "aether-campus-os")
- [ ] Set up `babel.config.js` with path aliases (`@components/`, `@services/`, `@hooks/`, `@store/`, `@constants/`)
- [ ] Create `.env`:
  ```
...
- [ ] `hooks/useAuth.ts` — Auth hook: check JWT from `expo-secure-store`, redirect to login if missing
- [ ] `hooks/useWebSocket.ts` — Socket.IO client hook (connect to `EXPO_PUBLIC_WS_URL`, auto-reconnect with `reconnectionDelay: 1000`)
- [ ] `hooks/useNotifications.ts` — Expo Notifications setup + FCM push token registration
- [ ] `store/auth.store.ts` — Zustand store: `user`, `token`, `setAuth()`, `clearAuth()`
- [ ] `store/notifications.store.ts` — Zustand store: notifications list, unread count
- [ ] `styles/global.ts` — Global StyleSheet with shared styles (no Tailwind)

### Common Components

- [ ] `components/common/Button.tsx` — Primary/secondary/outline variants using design tokens
- [ ] `components/common/Card.tsx` — Surface card with elevation using `COLORS.surface`
- [ ] `components/common/Badge.tsx` — Status badges (pending/approved/rejected)
- [ ] `components/common/Avatar.tsx` — User avatar circle
- [ ] `components/common/LoadingSpinner.tsx` — Animated spinner with primary color
- [ ] `components/common/NotificationBanner.tsx` — Push notification toast
